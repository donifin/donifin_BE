...
exports.getNews = onRequest(async (req, res) => {
  ...
  try {
    ...
    if (fetchAll || category === "환율") {
      result.exchange_rate = await fetchExchangeRate();
    }
    if (fetchAll || category === "주식") {
      result.stocks = await fetchStockChart();
    }
    if (fetchAll || category === "경제뉴스") {
      result.news = await fetchEconomyNews("경제");
    }
    if (fetchAll || category === "금리") {
      result.interest_rate = await fetchInterestRate();
    }
